# ADR-012: Uruchomienie wyłącznie z kodu źródłowego
...
przez PyInstaller i instalowany przez Inno Setup, a osoba techniczna instaluje
pakiet z PyPI poleceniem `pip install finddocs`. Wynikały z tego cztery koszty,
...
* wersja aplikacji żyła w czterech miejscach naraz (kod, metadane pakietu,
  skrypt instalatora, generator SBOM), a każde wydanie wymagało utrzymania
  publikacji na PyPI razem z jej dodatkami i presetami.
...
dla samego FindDocs, nie ma sekcji `[project]` w `pyproject.toml` ani punktów
...
(konwersja modeli), `requirements-gpu-dml.txt` i `requirements-gpu-cuda.txt`
(obliczenia na karcie graficznej), `requirements-pgvector.txt` (magazyn
wektorów w PostgreSQL), `requirements-dev.txt` (testy i linting). Pełne drzewo
...
* wersja aplikacji jest utrzymywana w jednym miejscu (`finddocs/version.py`),
  bo zniknęły stałe w pliku instalatora, w metadanych pakietu i w generatorze
  SBOM;
...
* dodatki i presety pip zastąpiły pliki `requirements*.txt`, które widać
  w repozytorium bez sięgania do metadanych pakietu.
