...
FindDocs nie ma instalatora, pliku wykonywalnego ani pakietu na PyPI.
Aplikację uruchamia się wprost z kodu źródłowego, interpreterem Pythona
ze środowiska wirtualnego przygotowanego obok kodu. Ten rozdział opisuje
przygotowanie środowiska, pierwsze uruchomienie, konfigurację oraz podłączenie
modelu embeddingów.
...
Zależności dzielą się na pliki, z których obowiązkowy jest tylko pierwszy:
...
| `requirements-export.txt` | torch, transformers, onnx i onnxscript: konwersja modeli Hugging Face do ONNX oraz kwantyzacja INT8 |
| `requirements-gpu-dml.txt` | wariant DirectML pakietu onnxruntime: embeddingi na karcie AMD, Intel albo NVIDIA na Windows |
| `requirements-gpu-cuda.txt` | wariant CUDA pakietu onnxruntime dla kart NVIDIA, razem z bibliotekami CUDA i cuDNN z pip (pakiety `nvidia-*`, około 2 GB) |
| `requirements-pgvector.txt` | sterownik psycopg: magazyn wektorów w bazie PostgreSQL z rozszerzeniem pgvector |
...
| Stanowisko | Polecenia |
| --- | --- |
| biurowe, obliczenia na CPU | `requirements.txt` + `requirements-ocr.txt` |
| dowolna karta graficzna na Windows | to samo + `requirements-gpu-dml.txt` |
| karta NVIDIA | to samo + `requirements-gpu-cuda.txt` |
| indeksowanie do wspólnej bazy | to samo + `requirements-pgvector.txt` |

### Instalacja wariantu GPU

Pliki `requirements-gpu-dml.txt` i `requirements-gpu-cuda.txt` instalują wariant
GPU pakietu onnxruntime obok wariantu CPU z `requirements.txt`. Oba wydania mają
ten sam pakiet importowy, a pip nie gwarantuje, które koło zapisze pliki jako
ostatnie, więc instalacja jest zawsze dwuetapowa:

```bat
.venv\Scripts\python -m pip install -r requirements-gpu-cuda.txt
.venv\Scripts\python -m pip install --force-reinstall --no-deps onnxruntime-gpu
```

Dla DirectML drugie polecenie brzmi
`pip install --force-reinstall --no-deps onnxruntime-directml`. Stan sprawdza
`run.py model device`. Wariant CUDA przynosi biblioteki CUDA i cuDNN jako zwykłe
pakiety pip, więc poza aktualnym sterownikiem NVIDIA nic nie trzeba instalować
ręcznie. Urządzenie obliczeń włącza `run.py model device cuda` (albo `dml`,
albo `auto`) lub karta **Obliczenia embeddingów** w GUI. Szczegóły, wersje
...
* `vector_store`: magazyn wektorów, opisany w
  [rozdziale o bazie wektorowej](baza-wektorowa.md),
...
i jest zalecana przy obliczeniach na karcie graficznej. Katalog modelu musi
zawierać `manifest.json`; katalogi bez manifestu importuje się poleceniem
`run.py model import`, które go utworzy.
